...
// Perform t-SNE
void TSNE::run(double* X, int N, int D, double* Y, int no_dims, double perplexity, double theta, double (*metrics)(const DataPoint&, const DataPoint&)) {    
  ...
  if(exact) {
    ...
    computeGaussianPerplexity(X, N, D, P, perplexity, metrics);
  ...
  // Compute input similarities for approximate t-SNE
    ...
    computeGaussianPerplexity(X, N, D, &row_P, &col_P, &val_P, perplexity, (int) (3 * perplexity), metrics);
...
void TSNE::computeGaussianPerplexity(double* X, int N, int D, double* P, double perplexity, double (*metrics)(const DataPoint&, const DataPoint&)) {
  double* DD = (double*) malloc(N * N * sizeof(double));
  if(DD == NULL) { printf("Memory allocation failed!\n"); exit(1); }
  if (metrics == euclidean_distance)
    computeSquaredEuclideanDistance(X, N, D, DD);
  else {
    // Computes distance with supllied distance function
    for (int n1 = 0; n1 < N; n1++) {
      for (int n2 = 0; n2 < N; n2++) {
	if (n2 > n1) {
	  DataPoint x1(D, n1, X + n1 * D);
	  DataPoint x2(D, n2, X + n2 * D);
	  DD[n1 * N + n2] = metrics(x1, x2);
	  if (DD[n1 * N + n2] < DBL_MIN) DD[n1 * N + n2] = DBL_MIN;
	} else if (n1 > n2) {
	  DD[n1 * N + n2] = DD[n2 * N + n1];
	} else {
	  DD[n1 * N + n2] = DBL_MIN;
	}
      }
...
void TSNE::computeGaussianPerplexity(double* X, int N, int D, int** _row_P, int** _col_P, double** _val_P, double perplexity, int K, double (*metrics)(const DataPoint&, const DataPoint&)) {
    ...
    *_val_P = (double*) calloc(N * K, sizeof(double));
    ...
    VpTree<euclidean_distance>* tree = new VpTree<euclidean_distance>();
